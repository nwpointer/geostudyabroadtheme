...
```js
var sassMap = fs.readFileSync('from.css.map');
processor.process(css, { map: sassMap, from: 'from.css', to: 'to.css' });
```
...
var result = minifier.process(css, {
    map:   fs.readFileSync('main.css.map'),
    from: 'main.css',
    to:   'main.min.css'
});

result.map //=> Source map from main to main.min.css
```
